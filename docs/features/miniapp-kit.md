...
## 视觉风格（ui-4 软陶毡面，2026-09-09 用户选定）

全包插画锚点是 `art/generated-art/me-heal-preview-ui/ui-4.png`。`art.config.json` 的 `style` 与 `art/prompts.txt` 头注释必须与此一致；新提示词用 `--ref` 指这份稿。

- **气质**：软陶 / 毡面 3D（claymorphism），像可拿起来的圆润小物件，细腻纸纹与毡面颗粒，柔和体积光，无硬描边。
- **禁止**：扁平矢量色块、水彩勾线、彩铅排线、粘土写实照片、硬描边描线。
- **色板**：奶油白、燕麦 `#f7f4ee`、米色胶囊 `#e9dfd0`、雾蓝、陶土橘、浅金褐。
- **品牌栏**：雾蓝圆角卡铺满画面，右侧奶油猫坐桃色月牙，左侧白字「测测子」「来测测你的另一面」画进图（喜欢稿上的圆润无衬线），不要 CSS 叠字。
