...
- 时间：2026-09-14 15:29
- 原因：COS 题库未上传、SHA 新目录与玩家指针错位、同名图缓存、频道名应对齐 tag。
- 修改：`cos-assets.md` / `miniapp-kit.md` 补热更指针、`assetRev`、`--channel` 读 tag、升文件名不再强制。发布脚本 `assets:hot` / `assets:registry` / `assets:channel`。kit `cos/` 同步 `--channel` 与 `--cache short`。
- 验证：见仓库根 `context.md` 2026-09-14 条。
