...
- 新页面 `pages/privacy`：隐私政策与用户条款静态文案（本地存储说明、微信广告与客服的第三方说明、娱乐向免责）。入口：我的 → 设置 → 隐私政策与用户条款。
- 新页面 `pages/settings`：版本号（`APP_VERSION`，与 package.json 同步维护）、隐私条款入口、清空测试记录 / 塔罗历史（`showModal` 二次确认）。
