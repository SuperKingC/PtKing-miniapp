...
describe('share capability wiring', () => {
  ...
  it('wires the me-page entries: data management inline + contact sessions', () => {
    ...
    expect(me).toContain('me-banner-baked-shadow-v1.jpg')
    expect(me).toContain('className="me-page__banner-img" src={meBannerImg} mode="aspectFit"')
    expect(me).toContain('版本 {APP_VERSION}')
    expect(me).toContain('icon-me-clear-v4.png')
    expect(me).toContain('icon-me-theme-v4.png')
    expect(me).toContain('icon-me-haptics-v4.png')
    expect(me).not.toContain('icon-me-clear-v5.png')
  ...
  it('fills the me-page brand card and keeps version in the footer', () => {
    ...
    expect(styles).toMatch(/\.me-page__banner \{[^}]*height: 0;[^}]*padding-bottom: 52%/)
    expect(styles).toMatch(/\.me-page__banner-img \{[^}]*top: -12\.5%;[^}]*right: -17%;[^}]*bottom: -12\.189826%;[^}]*left: -17%;[^}]*width: 134%;[^}]*height: 124\.689826%/)
    ...
    // 品牌图自带阴影，列表和偏好卡使用 CSS 阴影，图标不再叠加滤镜。
    expect(styles).toMatch(/\.me-page__entries \{[^}]*box-shadow:/)
    expect(styles).toMatch(/\.me-page__prefs \{[^}]*box-shadow:/)
    expect(appStyles).toContain('--shadow-slab:')
    expect(styles).toMatch(/\.me-page__icon \{[^}]*filter: none/)
    expect(styles).toMatch(/\.me-page__icon \{[\s\S]*?width: 72rpx;[\s\S]*?height: 72rpx/)
