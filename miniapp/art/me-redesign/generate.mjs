...
// 「我的」页重设计批次：展开 prompts-en.txt 的 {STYLE} → 临时配置(放开文字禁则/关压缩/放大限额)
...
baseCfg.prompt.bans = baseCfg.prompt.bans.filter((b) => !/文字|字母|数字/.test(b)) // 横幅标题需要
...
await new Promise((resolve, reject) => {
  // gen.mjs 的 --ratio 是全局参数，本批次横幅/图标比例不同：由提示词控构图，prepare 阶段裁到目标比例
  const child = spawn(process.execPath, [kitGen, '-c', configPath, '-p', promptsPath, '--out', out, '--ref', path.join(repo, 'miniapp/art/ref-pages-v3/reference-ui.png')], { stdio: 'inherit', cwd: repo })
