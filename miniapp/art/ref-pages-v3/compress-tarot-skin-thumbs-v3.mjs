// TinyPNG 压牌桌预览 v3（prepared/ → src/assets/illus）
...
const names = ['tarot-skin-clay-v3.jpg', 'tarot-skin-classic-v2.jpg']
for (const name of names) {
  ...
  for (const key of keys) {
    ...
    if (!response.ok) throw new Error(`${name} shrink HTTP ${response.status}`)
    const { output } = await response.json()
    const downloaded = await fetch(output.url, { headers: { Authorization: auth }, signal: AbortSignal.timeout(120000) })
    if (!downloaded.ok) throw new Error(`${name} download HTTP ${downloaded.status}`)
  ...
  if (!done) throw new Error(`no available TinyPNG key for ${name}`)
}
