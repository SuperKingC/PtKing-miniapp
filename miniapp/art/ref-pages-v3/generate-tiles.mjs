// 三枚（+MBTI 备用）tile 重生驱动：展开 STYLE → kit gen（--ref reference-ui.png）
...
baseCfg.prompt.bans = baseCfg.prompt.bans.filter((b) => !/文字|字母|数字/.test(b)) // MBTI 字母需要
...
const raw = fs.readFileSync(path.join(root, 'prompts-tiles.txt'), 'utf8')
...
const promptsPath = path.join(root, 'prompts-tiles-expanded.local.txt')
