// 一次性：TinyPNG 压缩 hero-card-v10（prepared/ -> src/assets/illus，升版文件名）。
// 输入 prepared/hero-card-v10.png 由 fix-hero-card-edge-v10.py 生成。
...
for (const key of keys) {
  ...
  const out = Buffer.from(await (await fetch(output.url, { headers: { Authorization: auth }, signal: AbortSignal.timeout(180000) })).arrayBuffer())
